  ...
  useEffect(() => {
    ...
    function create(this: Phaser.Scene) {
      ...
      this.physics.add.overlap(playersGroup, boostPads, (player: any, pad: any) => {
        // Host processes everyone to maintain authority.
        // Guests only process themselves for local prediction.
        if (!isHost && player.id !== participantId) return;

        if (!player.isBoosting) {
          player.isBoosting = true;
          if (player.id === participantId) {
            this.cameras.main.shake(300, 0.01);
            this.cameras.main.flash(300, 0, 255, 255, true);
          }
    ...
    function update(this: Phaser.Scene) {
      ...
      if (self) {
        ...
        const currentSpeed = self.boatBody.velocity.length();
        const speedMPH = Math.floor(currentSpeed / 5);
        speedText.setText(`${speedMPH} MPH`);
        speedText.setColor(self.isBoosting ? '#00ffff' : '#fbbf24');

        // Subtle vibration at high speeds
        if (currentSpeed > 400 && currentGameState === 'RACING') {
          const intensity = (currentSpeed - 400) * 0.000005;
